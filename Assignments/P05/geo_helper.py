...
class GeoHelper():
    ...
    def get_all_countries(self):
        return [{
            "status": 200 if self.country_names else 500,
            "description": "List of all countries in the world",
            "result": self.country_names
        }]
    ...
    def get_countries_by_continent(self, continent):
        ...
        if not country_list:
            raise HttpException(
                status_code = 404,
                details = "Invalid continent, check spelling",
                headers={"WWW-Authenticate": "Bearer"},
            )
        else:
            return [{"status": "200",
                     "description": "List of countries belonging to "
                                    "each continent",
    ...
    def calculate_distance(self, country0, country1):
        ...
        return [{
            "status": "200" if distance else "404",
            "description": "Distance from country0 to country1",
            "results": f'{round(distance,0)} miles'
        }]
    ...
    def calculate_bearing(self, country0, country1):
        ...
        for (x1, y1), (x2, y2) in zip(country_distance, country_distance[1:]):
            ...
            return [{
                "status": 200 if brng else 404,
                "description": "Calculate the bearing between two countries",
                "results": str(brng)
            }]
